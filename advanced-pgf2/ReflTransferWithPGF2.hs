...
-- The subject and the object must be the same abstract syntax function.
  ...
  main = do
    ...
    let conc = languages gr ! "MiniLangEng" -- NB. consider being safer in your application
        typ = startCat gr
    ...
    loop (translate transfer conc typ)
  ...
  translate :: (Expr -> Expr) -> Concr -> Type -> String -> String
  translate tr conc typ s = case parse conc typ s of
    ParseOk ((t,_):_) -> linearize conc (tr t)
    _ -> "NO PARSE"
